...
{
  UBYTE *pubSecond = pubMask + pixSizeU * pixSizeV; // remember pointer after first mip map
...
void CLayerMaker::CalculateData(void)
{
  ...
  lm_pbpoPolygon->bpo_mdShadow.GetSpaceCoordinates(lm_pbpoPolygon->bpo_pbplPlane->bpl_pwplWorking->wpl_mvRelative,
      vmex0, lm_vO);
...
void CLayerMaker::SpreadShadowMaskOutwards(void)
{
  ...
  //      ADDNEIGHBOUR(+0, +0);
          ...
          // otherwise make this one shadowed
          } else {
...
void CLayerMaker::MakePolygonMask(void)
{
  ...
  for (INDEX iMipmap = 0; iMipmap<lm_mmtPolygonMask.mmt_ctMipmaps; iMipmap++)
  {
    ...
    for (PIX pixV = 0; pixV < lm_pixSizeV >> iMipmap; pixV++)
    {
      ...
      {
        // if the point is in the polygon then set the pixel
        if (TestPointInPolygon(*lm_pbpoPolygon, vPoint)) {
          *pub = 255;

        // if the point is not in the polygon then clear the pixel
        } else {
          *pub = 0;
        }
...
{
  // if the light doesn't cast shadows, or the polygon does not receive them then do nothing
...
ULONG CLayerMaker::MakeOneShadowMaskMip(INDEX iMip)
{
  ...
  if (lm_plsLight->ls_ulFlags & LSF_DIRECTIONAL) {
    ...
    prProjection.ScreenBBoxL() = FLOATaabbox2D(
      FLOAT2D(pixLayerMinU, pixLayerMinV),
      FLOAT2D(pixLayerMinU + pixLayerSizeU, pixLayerMinV + pixLayerSizeV)
    );
    ...
    // if polygon is turned away from the light then do nothing
    ...
    ulLighted &= RenderShadows(*lm_pwoWorld, *(CEntity*)NULL, apr,
      lm_pbpoPolygon->bpo_boxBoundingBox, pubLayer, pixLayerSizeU, pixLayerSizeV, lm_plsLight->ls_ubPolygonalMask);
  ...
  // if the light is point
    ...
    if (!(lm_pbpoPolygon->bpo_ulFlags & BPOF_DARKCORNERS)) {
      prProjection.ScreenBBoxL() = FLOATaabbox2D(
        FLOAT2D(pixLayerMinU - fpixHotU + 1, pixLayerMinV - fpixHotV + 1),
        FLOAT2D(pixLayerMinU + pixLayerSizeU - fpixHotU + 1, pixLayerMinV + pixLayerSizeV - fpixHotV + 1)
      );

    } else {
      prProjection.ScreenBBoxL() = FLOATaabbox2D(
        FLOAT2D(pixLayerMinU - fpixHotU, pixLayerMinV - fpixHotV),
    ...
    prProjection.FarClipDistanceL() = lm_fLightPlaneDistance-lm_plsLight->ls_fFarClipDistance; //-fEpsilon/2; !!!! use minimal epsilon for polygon
    ...
    if (lm_plsLight->ls_ubPolygonalMask == 0) {
      ...
      ulLighted &= RenderShadows(*lm_pwoWorld, *lm_plsLight->ls_penEntity, apr, FLOATaabbox3D(),
...
//  __pfWorldEditingProfile->StartTimer(CWorldEditingProfile::PTI_MAKESHADOWS);
  ...
  {
    // if already calculated, or dynamic then skip it
    if (itbsl->bsl_ulFlags & BSLF_CALCULATED || itbsl->bsl_plsLightSource->ls_ulFlags & LSF_DYNAMIC) {
      continue;
    }

    // if we are not doing directional lights and it is directional then skip it
    ...
    // if the layer is not needed then free it
  ...
  // if was intialized then free bit-packed polygon mask
  if (bInitialized) {
    FreeMemory(lm_pubPolygonMask);
  }

  // if some new layers have been calculated then invalidate mixed layers
  if ( bCalculatedSome) {
